      ...
      <main class="h-full overflow-y-auto">
        <div class="container px-6 mx-auto grid">
          ...
          <a class="mt-6 flex items-center justify-between p-4 mb-8 text-sm font-semibold text-white bg-sp-primary-400 rounded-lg shadow-md focus:outline-none focus:shadow-outline-purple"
            href="/daftar-guru/tambah" id="add-teacher">
            <div class="flex items-center gap-2">
              <span><?php count($teachers); ?> Guru</span>
            </div>
              ...
              <table class="w-full whitespace-no-wrap">
                <thead>
                  <tr
                    ...
                    <th class="px-4 py-3 w-2/12">NIP</th>
                ...
                <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                  ...
                  <tr class="text-gray-700 dark:text-gray-400">
                    <td class="px-4 py-3">
                      <div class="flex items-center text-sm cursor-pointer">
                        ...
                        <div class="relative hidden w-8 h-8 mr-3 rounded-full md:block text-center">
                          <img class="object-cover w-full h-full rounded-full" src="<?= $teacher['photo'] ?>" alt=""
                            loading="lazy" />
                        ...
                        <?php } ?>
                        <div>
                          <p class="font-semibold"><?= $teacher['name'].', '.$teacher['degree']  ?></p>
                          <p class="text-xs text-gray-600 dark:text-gray-400">
                            <?php if ($teacher['gender'] == 'L') { ?>
                            Laki-laki
                            <?php } else if ($teacher['gender'] == 'P') { ?>
                            Perempuan
                            <?php } ?>
                          </p>
                        </div>
                    ...
                    <td class="px-4 py-3 text-sm flex gap-3">
                      <a
                        ...
                        href="/daftar-guru/edit?page=guru&teacher_id=<?= $teacher['teacher_id'] ?>">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                          stroke="currentColor">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                      ...
                      <button
                        ...
                        id="delete-teacher" data-id="<?= $teacher['teacher_id'] ?>">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                          stroke="currentColor">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                  ...
                  <div
                    ...
                    id="modal-delete-teacher-<?= $teacher['teacher_id'] ?>">
                    <div
                      ...
                      <header class="flex justify-end">
                        <button
                          ...
                          aria-label="close" id="modal-close-button" data-id="<?= $teacher['teacher_id'] ?>">
                      ...
                      <div class="mt-4 mb-6">
                        ...
                        <p class="text-sm text-gray-700 dark:text-gray-400">
                          Apakah Anda ingin menghapus data <?= $teacher['name'] ?>?
                        </p>
                      </div>

                      <footer
                        class="flex flex-col items-center justify-end gap-2 sm:flex-row bg-gray-50 dark:bg-gray-800">
                        <button id="modal-close-button" data-id="<?= $teacher['teacher_id'] ?>"
  ...
  <script type="text/javascript">
    $(document).ready(function () {
      $("body").on('click', "#delete-teacher", function (e) {
        ...
        $(`#modal-delete-teacher-${teacherId}`).removeClass('hidden').addClass('flex');
      });

      $("body").on('click', "#modal-close-button", function (e) {
        var teacherId = $(this).data(("id"));
        $(`#modal-delete-teacher-${teacherId}`).removeClass('flex').addClass('hidden');
      });

      $("body").on('click', "#update-teacher", function (e) {
        var teacherId = $(this).data("id");
        $(`#modal-update-teacher-${teacherId}`).removeClass('hidden').addClass('flex');
      });

      $("body").on('click', "#update-teacher-close", function (e) {
        var teacherId = $(this).data("id");
        $(`#modal-update-teacher-${teacherId}`).removeClass('flex').addClass('hidden');
      });
