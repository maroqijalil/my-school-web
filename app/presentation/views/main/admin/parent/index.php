...
<body>
      ...
      <main class="h-full overflow-y-auto">
        <div class="container px-6 mx-auto grid">
          ...
          <a class="mt-6 flex items-center justify-between p-4 mb-8 text-sm font-semibold text-white bg-sp-primary-400 rounded-lg shadow-md focus:outline-none focus:shadow-outline-purple"
            href="/daftar-ortu/tambah" id="add-parent">
            <div class="flex items-center gap-2">
              <span><?php count($parents); ?> Ortu</span>
            </div>
              ...
              <table class="w-full whitespace-no-wrap">
                ...
                <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                  ...
                  <tr class="text-gray-700 dark:text-gray-400">
                    ...
                    <td class="px-4 py-3 text-sm flex gap-3">
                      <a
                        ...
                        href="/daftar-ortu/edit?page=ortu&parent_id=<?= $parent['parent_id'] ?>">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                          stroke="currentColor">
                      ...
                      <button
                        ...
                        id="delete-parent" data-id="<?= $parent['parent_id'] ?>">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                          stroke="currentColor">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                  ...
                  <div
                    ...
                    id="modal-delete-parent-<?= $parent['parent_id'] ?>">
                    <div
                      ...
                      <header class="flex justify-end">
                        <button
                          ...
                          aria-label="close" id="modal-close-button" data-id="<?= $parent['parent_id'] ?>">
                      ...
                      <div class="mt-4 mb-6">
                        ...
                        <p class="text-sm text-gray-700 dark:text-gray-400">
                          Apakah Anda ingin menghapus data <?= $parent['name'] ?>?
                        </p>
                      </div>

                      <footer
                        class="flex flex-col items-center justify-end gap-2 sm:flex-row bg-gray-50 dark:bg-gray-800">
                        <button id="modal-close-button" data-id="<?= $parent['parent_id'] ?>"
  ...
  <script type="text/javascript">
    $(document).ready(function () {
      $("body").on('click', "#delete-parent", function (e) {
        var parentId = $(this).data(("id"));
        $(`#modal-delete-parent-${parentId}`).removeClass('hidden').addClass('flex');
      });

      $("body").on('click', "#modal-close-button", function (e) {
        var parentId = $(this).data(("id"));
        $(`#modal-delete-parent-${parentId}`).removeClass('flex').addClass('hidden');
      });

      $("body").on('click', "#update-parent", function (e) {
        var parentId = $(this).data("id");
        $(`#modal-update-parent-${parentId}`).removeClass('hidden').addClass('flex');
      });

      $("body").on('click', "#update-parent-close", function (e) {
        var parentId = $(this).data("id");
        $(`#modal-update-parent-${parentId}`).removeClass('flex').addClass('hidden');
      });
